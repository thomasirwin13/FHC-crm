      ...
      {expanded && (
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            ...
            letters, emails, and events in your Action Network account.
          </p>
                ...
                <div className="space-y-2">
                  {filteredActions.map((action) => (
                    <ActionRow
                      ...
                      action={action}
                      open={openAction === `${action.type}-${action.id}`}
                      onToggle={() =>
                        setOpenAction((prev) =>
                          prev === `${action.type}-${action.id}` ? null : `${action.type}-${action.id}`
                        )
                      }
                    />
                  ))}
...
function ActionRow({
  ...
  return (
    <div className="rounded-md border border-border/50">
      <button
        onClick={onToggle}
        className="w-full flex items-center gap-3 px-3 py-2.5 text-left hover:bg-muted/20 transition-colors rounded-md"
      >
            ...
            <>
              ...
              {action.resolved < action.total && (
                <p className="text-xs text-muted-foreground mt-1.5">
                  ...
                  {action.resolved < action.total && ' (some could not be resolved to a person record)'}.
                </p>
              )}
            </>
          )}
