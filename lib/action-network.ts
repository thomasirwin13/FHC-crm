...
//
// Requires ACTION_NETWORK_API_KEY in the environment. Action Network
// authenticates with an `OSDI-API-Token` header (not a Bearer token) and
// returns OSDI/HAL resources under `_embedded` with `osdi:*` keys.
//
...
function getApiKey(): string | null {
  return process.env.ACTION_NETWORK_API_KEY || null;
}
...
async function anFetch(urlOrPath: string, init?: RequestInit) {
  const key = getApiKey();
  if (!key) throw new Error('Action Network is not configured (missing ACTION_NETWORK_API_KEY)');
...
/** Fetch every activist in the group, following HAL `next` links. */
export async function fetchAllPeople(): Promise<{ people: ANPerson[]; capped: boolean }> {
  const out: ANPerson[] = [];
...
export function fetchPetitions(): Promise<ANActionResource[]> {
  return fetchActionList('/petitions', 'osdi:petitions');
}

export function fetchEvents(): Promise<ANActionResource[]> {
...
export function fetchSignaturePersonIds(petitionId: string): Promise<ANActionMembers> {
  return fetchActionMembers(`/petitions/${petitionId}/signatures`, 'osdi:signatures');
}

export function fetchAttendancePersonIds(eventId: string): Promise<ANActionMembers> {
  return fetchActionMembers(`/events/${eventId}/attendances`, 'osdi:attendances');
}

/**
 * Create or update a person by email. Action Network upserts on email when you
 * POST to the people collection, so this is safe for contacts that may already
 * exist. Optionally applies tags (created in Action Network on demand).
 */
export async function upsertPerson(contact: {
  email: string;
  name: string | null;
  phone: string | null;
  city: string | null;
  state: string | null;
  zip: string | null;
}, addTags: string[] = []): Promise<{ ok: boolean; error?: string }> {
